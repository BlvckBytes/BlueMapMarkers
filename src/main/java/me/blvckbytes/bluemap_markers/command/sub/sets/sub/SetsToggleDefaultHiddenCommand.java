...
public class SetsToggleDefaultHiddenCommand extends SubCommand {
  ...
  public SetsToggleDefaultHiddenCommand(MarkerSetStore markerSetStore) {
  ...
  public @Nullable CommandFailure onCommand(CommandSender sender, String[] args, Queue<NormalizedConstant<?>> actions) {
    ...
    var newValue = !markerSet.defaultHidden();
    var result = markerSetStore.setMarkerSetDefaultHidden(name, newValue);

    switch (result.status) {
      case SUCCESS -> sender.sendMessage("§aThe default-hidden-value of the marker-set " + name + " has been successfully set to " + newValue);
